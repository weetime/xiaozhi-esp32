...
void AudioCodec::Start() {
    ...
    // Save original output sample rate.
...
bool AudioCodec::SetOutputSampleRate(int sample_rate) {
    // Special handling: -1 means reset to original sample rate.
    if (sample_rate == -1) {
        if (original_output_sample_rate_ > 0) {
    ...
    // Try to disable I2S channel first (if enabled).
    ...
    if (disable_ret == ESP_OK) {
        ...
        // Channel might already be disabled, which is normal.
    ...
    // Re-enable channel (need to enable for audio playback regardless of previous state).
